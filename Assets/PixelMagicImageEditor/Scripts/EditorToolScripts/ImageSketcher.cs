namespace PixelMagicEditor
{
    ...
    public static class ImageSketcher
    {
        ...
        private static Texture2D ApplyEdgeDetection(Texture2D inputTexture, float edgeDetectionThreshold)
        {
            ...
            for (int x = 1; x < inputTexture.width - 1; x++)
            {
                for (int y = 1; y < inputTexture.height - 1; y++)
                {
                    ...
                    Color[] neighbors =
                    {
                        inputTexture.GetPixel(x - 1, y - 1), inputTexture.GetPixel(x, y - 1),
                        inputTexture.GetPixel(x + 1, y - 1),
        ...
        private static Texture2D ApplySketchEffect(Texture2D inputTexture, float edgeDetectionThreshold,
            Color sketchColor)
        {
